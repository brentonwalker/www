...
function siteorigin_premium_admin_menu() {
	...
	add_theme_page( __( 'Premium Upgrade', 'focus' ), __( 'Premium Upgrade', 'focus' ), 'switch_themes', 'premium_upgrade', 'siteorigin_premium_page_render' );
}
	...
	switch ( $action ) {
		...
		case 'enter-order' :
			...
			<div class="wrap" id="theme-upgrade">
				<h2><?php printf(__('Your Order Number Is [%s]', 'focus'), get_option( $option_name )) ?></h2>

				<?php if ( is_null( $valid ) ) : ?>
				<p>
					<?php _e( "There was a problem contacting our validation servers.", 'focus' ) ?>
					<?php _e( "Please try again later, or upgrade manually using the ZIP file we sent you.", 'focus' ) ?>
				</p>
				<?php elseif ( $valid ) : ?>
				<p>
					<?php _e( "We've validated your order number.", 'focus' ) ?>
					<?php
					printf(
						__( 'You can update now, or later on your <a href="%s">Themes page</a>.', 'focus' ),
						admin_url( 'themes.php' )
					);
					?>
					<?php _e( 'This update will add all the premium features.', 'focus' ) ?>
				</p>
				<p class="submit">
					<?php
					$update_url = wp_nonce_url( admin_url( 'update.php?action=upgrade-theme&amp;theme=' . urlencode( $theme ) ), 'upgrade-theme_' . $theme );
					$update_onclick = 'onclick="if ( confirm(\'' . esc_js( __( "Updating this theme will lose any code customizations (CSS, PHP, Javascript, etc) you have made to the free version. You will not lose your content, images or settings. 'Cancel' to stop, 'OK' to update.", 'focus' ) ) . '\') ) {return true;}return false;"';
					?>
					<a href="<?php echo esc_url( $update_url ) ?>" <?php echo $update_onclick ?> class="button-primary">
						<?php _e( 'Update Theme', 'focus' ) ?>
					</a>
				</p>
				<?php else : ?>
				<p>
					<?php _e( "We couldn't validate your order number.", 'focus' ) ?>
					<?php _e( "There might be a problem with our validation server.", 'focus' ) ?>
					<?php _e( "Please try again later, or upgrade manually using the ZIP file we sent you.", 'focus' ) ?>
				</p>
				<?php endif; ?>
			</div>
	...
	if( siteorigin_premium_teaser_get_support('page-templates') ){
		...
		wp_localize_script( 'siteorigin-premium-teaser-templates', 'siteoriginTeaserTemplates' , array(
			'code' => '<p>'.siteorigin_premium_teaser(
				__('Get Additional Templates', 'focus'),
				array(
					'description' => sprintf(__('%s Premium includes additional templates', 'focus'), ucfirst(get_option('stylesheet')))
				),
				true
			).'</p>'
		) );
...
function siteorigin_premium_call_function($callback, $param_array, $args = array()){
	if(function_exists($callback) && defined('SITEORIGIN_IS_PREMIUM')){
		...
		<a class="siteorigin-premium-teaser" href="<?php echo admin_url( 'themes.php?page=premium_upgrade' ) ?>" target="_blank">
			<em></em>
			<?php printf( __( 'Available in <strong>%s Premium</strong> - <strong class="upgrade">Upgrade Now</strong>', 'focus' ), ucfirst($theme) ) ?>
...
class SiteOrigin_Premium_Teaser_Customizer extends WP_Customize_Section{
	function render() {
		...
		siteorigin_premium_teaser(__('Get Additional Customizations', 'focus'));
		?></div><?php
	}
}
endif;
...
function siteorigin_premium_default_content($content){
	...
	$content['rewards'][] = array(
		'amount' => 10,
		'title' => sprintf(__('A Copy of %s Premium', 'focus'), ucfirst($theme)),
		'text' => sprintf(__('You get a copy of %s Premium, delivered instantly to your PayPal email address. This includes the same basic support we offer users of our free themes.', 'focus'), ucfirst($theme)),
	);

	$content['rewards'][] = array(
		'amount' => 20,
		'title' => __('Premium Support', 'focus'),
		'text' => sprintf(__("This includes Premium email support from our support team for a single site. We'll help you as quickly as possible with all your setup and configuration questions.", 'focus'), ucfirst($theme)),
	);

	$content['rewards'][] = array(
		'amount' => 40,
		'title' => __('Advanced Premium Support', 'focus'),
		'text' => sprintf(__("We'll go the extra mile and help you with minor CSS customizations, plugin conflicts and anything else that falls outside standard %s Premium support.", 'focus'), ucfirst($theme)),
	);

	$content['rewards'][] = array(
		'amount' => 60,
		'title' => __('A Special Thank You', 'focus'),
		'text' => sprintf(__("Our highest level of support. If you need it, you'll get support directly from the developer of %s. We'll also include your name on our contributors list.", 'focus'), ucfirst($theme)),
	);
