 ...
 * @return string
 */
function toothpaste_gallery($contents, $attr){
	...
	foreach($attachments as $attachment){
		$return .= '<li>';
		$return .= apply_filters('toothpaste_slide_before', '', $attachment);
		...
		$return .= apply_filters('toothpaste_slide_after', '', $attachment);
		$return .= '</li>';
	}
	$return .= '</ul>';
	$return .= '</div>';
...
add_filter('post_gallery', 'toothpaste_gallery', 10, 2);
 ...
 */
function toothpaste_gallery_types($types){
	$types['slider'] = __('Slider', 'toothpaste');
	return $types;
}
add_filter('siteorigin_panels_gallery_types', 'toothpaste_gallery_types');
 ...
 * @since toothpaste 1.0
 */
function toothpaste_gallery_default_type(){
	return 'slider';
}
add_filter('siteorigin_panels_gallery_default_type', 'toothpaste_gallery_default_type');