...
function book_header_style() {
	...
	<?php
		...
		if( is_single() || is_page() ) :
			...
			elseif ( ! empty( $header_image_path ) ) :

				$header_image = "background: url(" . $header_image_path . ") center 0 no-repeat; background-attachment: fixed;";
				$textcolor = "#000";
		...
		<?php
			...
			if ( ! empty( $header_image_path ) ) :
			 ...
			 ?>
			#masthead {
				background: url( <?php echo esc_url( $header_image_path ); ?> ) center 0 no-repeat;
				margin-top: 0;
				max-width: 100%;
...
?>
	...
	.appearance_page_custom-header #headimg {
		position: relative;
		border: none;
		<?php if ( ! empty( $header_image_path ) ) : ?>

		background: url( <?php echo esc_url( $header_image_path ); ?> ) 0 0 no-repeat;
