...
<style>
	...
	.post-option input, .post-option textarea, .post-option select {
		font-size: 12px;
	}
...
foreach ($section_options as $priority => $options) {
	foreach ($options as $option_id => $option) {
		...
		$args = array('name_attr' => "post-options[{$option_id}]", 'value' => $this -> get_post_option($post -> ID, $option_id));
