 ...
 * @return string
 */
function so_current_gallery($contents, $attr){
	...
	foreach($attachments as $attachment){
		$return .= '<li>';
		$return .= apply_filters('so_current_slide_before', '', $attachment);
		...
		$return .= apply_filters('so_current_slide_after', '', $attachment);
		$return .= '</li>';
	}
	$return .= '</ul>';
	$return .= '</div>';
...
add_filter('post_gallery', 'so_current_gallery', 10, 2);
 ...
 * @since so-current 1.0
 */
function so_current_gallery_types($types){
	$types['slider'] = __('Slider', 'so-current');
	return $types;
}
add_filter('siteorigin_gallery_types', 'so_current_gallery_types');
 ...
 * @since so-current 1.0
 */
function so_current_gallery_default_type(){
	return 'slider';
}
add_filter('siteorigin_gallery_default_type', 'so_current_gallery_default_type');