...
class CoLabsThemes_Backup {
	...
	function init () {
		if ( is_admin() && ( get_option( 'framework_colabs_backupmenu_disable' ) != 'true' ) ) {
            ...
            //add_action( 'admin_head', array( &$this, 'register_admin_head' ) );
		}
	} // End init()
...
<div id="colabs_options" class="wrap <?php if (get_bloginfo('text_direction') == 'rtl') { echo 'rtl'; } ?> colabs_backup">
	...
	function add_to_export_query ( $options, $count ) {
		...
		foreach( $options as $option ) {
			...
			if( isset( $option['id'] ) ) {
				...
				// Multicheck fields
				if ( ! is_array( $option['type'] ) && ( $option['type'] == 'multicheck' || $option['type'] == 'multicheck2' ) ) {
