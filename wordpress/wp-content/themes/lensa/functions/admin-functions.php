...
function colabs_image($args) {
	...
	elseif ( get_option( 'colabs_resize') == 'true' && empty($vt_image['url']) ) { 
		...
		foreach($src_arr as $key => $custom_field){
			...
			$img_link = '<img src="'. get_bloginfo( 'template_url'). '/functions/timthumb.php?src='. $custom_field .'&amp;w='. $width .'&amp;h='. $height .'&amp;zc=1&amp;q='. $quality . $alignment . '" alt="'.$alt.'" class="'. stripslashes($class) .'" '. $set_width . $set_height . ' />';
...
function colabsthemes_timthumb_update_page(){
        ...
        $url = admin_url( 'admin.php?page=colabsthemes_framework_update' );
       	...
       	if (isset($_POST['colabs_update_save'])) {
       		...
       		$filename = locate_template( 'functions/timthumb.php' );
